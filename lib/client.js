    ...
    function buildCss(config) {
      ...
      if (config.error && config.error.enabled) {
        ...
        for (var t2 = 0; t2 < toolKeys.length; t2++) {
          ...
          var sels2 = leadingSelectors(tool2);
          var hideErr = [];
          var showErr = [];
          for (var s3 = 0; s3 < sels2.length; s3++) {
            hideErr.push(iconSelector(sels2[s3], "error"));
            showErr.push(sels2[s3] + "::before");
          }
          lines.push(hideErr.join(",") + "{display:none}");
          lines.push(showErr.join(",") + "{" + itemStyle(err) + "}");
        }
      }
